...
export const ReportCrosstab: React.FC<{ pivot: ReportPivotResult; metric?: ReportMetricDefinition; onDrill?: (rowKey: string, columnKey?: string) => void }> = ({ pivot, metric, onDrill }) => (
  <div className="min-w-0 overflow-hidden rounded-2xl border border-rule bg-white shadow-panel" data-testid="report-crosstab">
    ...
    <div className="overflow-x-auto">
      <table className="w-full min-w-[680px] text-left text-xs">
        <thead className="bg-slate-50 text-[10px] font-bold text-slate-500"><tr><th className="sticky left-0 bg-slate-50 px-4 py-3">Hàng</th>{pivot.columns.map(column => <th key={column.key} className="px-4 py-3 text-right">{column.label}</th>)}<th className="px-4 py-3 text-right">Tổng</th></tr></thead>
        <tbody className="divide-y divide-slate-100">{pivot.rows.map(row => <tr key={row.key}><td className="sticky left-0 bg-white px-4 py-3 font-semibold text-slate-800">{row.label}</td>{pivot.columns.map(column => <td key={column.key} className="px-4 py-3 text-right text-slate-700">{onDrill ? <button type="button" onClick={() => onDrill(row.key, column.key)} className="underline-offset-2 hover:text-brand-600 hover:underline">{metricValue(row.values[column.key] || 0, metric)}</button> : metricValue(row.values[column.key] || 0, metric)}</td>)}<td className="px-4 py-3 text-right font-black text-brand-600">{onDrill ? <button type="button" onClick={() => onDrill(row.key)} className="underline-offset-2 hover:underline">{metricValue(row.total, metric)}</button> : metricValue(row.total, metric)}</td></tr>)}</tbody>
      </table>
    </div>
...
export const ReportChart: React.FC<{ result: ReportRunResult; metricKey: ReportMetricKey; metric?: ReportMetricDefinition; type: ChartType }> = ({ result, metricKey, metric, type }) => {
  ...
  if (!data.length) return <div className="rounded-2xl border border-rule bg-white p-5 text-xs text-slate-500">Không có dữ liệu để vẽ biểu đồ.</div>;
  const tooltip = <Tooltip formatter={(value: number) => metricValue(value, metric)} />;
  return <div className="h-80 rounded-2xl border border-rule bg-white p-4 shadow-panel" data-testid="report-chart" role="img" aria-label={`Biểu đồ ${metric?.label || 'báo cáo'}`}>
