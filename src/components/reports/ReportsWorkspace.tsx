...
const ZONE_SELECT_CLASS = 'min-h-9 w-full rounded-lg border border-slate-200 bg-white px-2 text-xs font-bold text-slate-700 outline-none focus:border-[#006b68]';
        ...
        <details className="group" open>
          ...
          <div className="border-t border-slate-200 p-3">
            ...
            <div className="mt-4 space-y-4">
              <section>
                <h3 className="mb-2 flex items-center gap-2 text-[11px] font-black uppercase tracking-wide text-slate-500"><Sparkles className="h-3.5 w-3.5" />Mẫu dựng sẵn</h3>
                <div data-testid="report-preset-list" className="space-y-1">{availablePresets.map(preset => <button
                  key={preset.id}
                  type="button"
                  onClick={() => applyTemplate(preset.id)}
                  className={`w-full rounded-lg px-2.5 py-2 text-left ${selectedTemplateId === preset.id ? 'bg-teal-100 text-[#006b68]' : 'text-slate-700 hover:bg-white'}`}
              ...
              <section>
                <h3 className="mb-2 flex items-center gap-2 text-[11px] font-black uppercase tracking-wide text-slate-500"><Columns3 className="h-3.5 w-3.5" />Chiều phân tích</h3>
              ...
              <section>
                <h3 className="mb-2 flex items-center gap-2 text-[11px] font-black uppercase tracking-wide text-slate-500"><Sigma className="h-3.5 w-3.5" />Chỉ số</h3>
      ...
      <div data-testid="report-canvas" className="min-w-0 p-4 sm:p-5">
        <div className="grid gap-3 lg:grid-cols-[minmax(220px,1fr)_auto] lg:items-end">
          ...
          <div className="flex flex-wrap gap-2">
            <button type="button" onClick={() => { setCopySourceId(undefined); setShowSaveForm(current => !current); }} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-slate-200 px-3 py-2.5 text-xs font-bold text-slate-600 hover:bg-slate-50"><Bookmark className="h-4 w-4" />Lưu cách xem</button>
            {selectedTemplateId && <button type="button" onClick={prepareCopy} className="min-h-11 rounded-xl border border-slate-200 px-3 text-xs font-bold text-slate-600 hover:bg-slate-50">Sao chép</button>}
            <button type="button" onClick={resetBuilder} className="min-h-11 rounded-xl border border-slate-200 px-3 text-xs font-bold text-slate-600 hover:bg-slate-50">Làm lại</button>
          </div>
        </div>
        ...
        <div className="mt-4 grid gap-px overflow-hidden rounded-xl border border-slate-200 bg-slate-200 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <DropZone zone="values" icon={<Sigma className="h-3.5 w-3.5" />} title="Giá trị" onDrop={dropInto}>
            <div className="flex flex-wrap gap-1">{query.metrics.map(key => <span key={key} className="inline-flex items-center gap-1 rounded-lg bg-teal-50 py-1 pl-2 pr-1 text-[11px] font-bold text-[#006b68]">
              {metricsByKey.get(key)?.label || 'Chỉ số'}
              <button type="button" onClick={() => removeMetric(key)} disabled={query.metrics.length < 2} className="rounded p-0.5 hover:bg-teal-100 disabled:opacity-30" aria-label={`Bỏ chỉ số ${metricsByKey.get(key)?.label || ''}`}><X className="h-3 w-3" /></button>
            </span>)}</div>
          </DropZone>
          <DropZone zone="filters" icon={<Filter className="h-3.5 w-3.5" />} title="Bộ lọc" onDrop={dropInto}>
            <div className="flex flex-wrap gap-1">
              {query.rules.map((rule, index) => <button key={`${rule.key}-${index}`} type="button" onClick={() => setOpenRuleIndex(current => current === index ? null : index)} className={`rounded-lg px-2 py-1 text-[11px] font-bold ${openRuleIndex === index ? 'bg-[#006b68] text-white' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'}`}>{describeRule(rule, fieldsByKey.get(rule.key), operatorsByKey.get(rule.operator)?.label)}</button>)}
              <button type="button" onClick={() => addRule()} disabled={!filterFields.length} className="inline-flex items-center gap-1 rounded-lg border border-dashed border-slate-300 px-2 py-1 text-[11px] font-bold text-[#006b68] hover:bg-teal-50 disabled:opacity-50"><Plus className="h-3 w-3" />Thêm điều kiện</button>
            </div>
          </DropZone>
        </div>
        ...
        {query.rules.length > 0 && <div className="mt-3 space-y-3" data-testid="report-filter-editor">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <span className="flex items-center gap-2 text-xs font-bold text-slate-700"><Filter className="h-3.5 w-3.5 text-[#006b68]" />Lọc dữ liệu<span className="font-medium text-slate-400">{query.rules.length}</span></span>
            {query.rules.length > 1 && <label className="flex items-center gap-2 text-[11px] font-semibold text-slate-600"><span>Điều kiện:</span><select aria-label="Cách ghép điều kiện" value={query.match} onChange={event => setQuery(current => ({ ...current, match: event.target.value as ReportRunRequest['match'] }))} className="min-h-9 rounded-lg border border-slate-200 bg-white px-2 text-xs font-bold text-slate-700"><option value="ALL">Đồng thời thỏa tất cả</option><option value="ANY">Thỏa ít nhất một điều kiện</option></select></label>}
          </div>
        ...
        <div className="mt-4 grid gap-3 border-t border-slate-100 pt-3 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_auto_auto] lg:items-end">
          ...
          <label className="space-y-1.5 text-xs font-bold text-slate-600"><span>Dashboard</span><select aria-label="Dashboard" value={selectedDashboardId} onChange={event => setSelectedDashboardId(event.target.value)} className={CONTROL_CLASS}><option value="">Chưa chọn</option>{dashboards.map(dashboard => <option key={dashboard.id} value={dashboard.id}>{dashboard.name}</option>)}</select></label>
          <button type="button" onClick={() => setShowDashboardForm(current => !current)} className="min-h-11 rounded-xl border border-slate-200 px-3 text-xs font-bold text-slate-600 hover:bg-slate-50">Tạo dashboard</button>
          <button type="button" onClick={() => void runReport()} className="min-h-11 rounded-xl bg-[#006b68] px-4 text-xs font-bold text-white hover:bg-[#005a57]">Xem báo cáo</button>
        </div>
...
const DropZone: React.FC<{
  ...
  return <div
    ...
    className={`p-3 transition ${over ? 'bg-teal-50 ring-2 ring-inset ring-[#006b68]' : 'bg-white'}`}
  >
    <span className="mb-2 flex items-center gap-2 text-[10px] font-black uppercase tracking-wide text-slate-500">{icon}{title}</span>
    {children}
  </div>;
};
...
const ExportColumnPicker: React.FC<{ fields: ReportFieldDefinition[]; selected: ReportFieldKey[]; onChange: (keys: ReportFieldKey[]) => void }> = ({ fields, selected, onChange }) => {
  ...
  return <details className="relative text-xs font-bold text-slate-600">
    <summary className={`${CONTROL_CLASS} flex cursor-pointer list-none items-center justify-between`}><span>Cột xuất</span><span className="text-[11px] font-bold text-[#006b68]">{selected.length} cột</span></summary>
    <div className="absolute z-20 mt-1 max-h-64 w-full min-w-[14rem] overflow-y-auto rounded-xl border border-slate-200 bg-white p-2 shadow-lg">
  ...
  return <div className="fixed inset-0 z-40 flex justify-end bg-slate-900/40" role="dialog" aria-modal="true" aria-label="Hồ sơ chi tiết" data-testid="report-drill-panel" onClick={onClose}>
    <div className="flex h-full w-full max-w-3xl flex-col bg-white shadow-2xl" onClick={event => event.stopPropagation()}>
      <div className="flex items-start justify-between gap-3 border-b border-slate-200 px-4 py-3">
        ...
        <button type="button" onClick={onClose} className="rounded-lg border border-slate-200 p-2 text-slate-500 hover:bg-slate-50" aria-label="Đóng hồ sơ chi tiết"><X className="h-4 w-4" /></button>
      </div>
      <div className="min-h-0 flex-1 overflow-auto">
        ...
        {!loading && result && result.rows.length > 0 && <table className="w-full min-w-[720px] text-left text-xs">
          <thead className="sticky top-0 bg-slate-50 text-[10px] font-bold uppercase tracking-wide text-slate-500"><tr>
          ...
          <tbody className="divide-y divide-slate-100">{result.rows.map(row => <tr key={row.findingId}>
            ...
            <td className="px-3 py-2 text-right font-bold text-[#006b68]">{row.exposureAmount.toLocaleString('vi-VN')} triệu</td>
          </tr>)}</tbody>
        </table>}
      </div>
      {result && result.total > result.pageSize && <div className="flex items-center justify-between gap-2 border-t border-slate-200 px-4 py-3 text-xs font-bold text-slate-600">
        <button type="button" disabled={result.page <= 1} onClick={() => onPage(result.page - 1)} className="min-h-9 rounded-lg border border-slate-200 px-3 disabled:opacity-40">Trước</button>
        <span>Trang {result.page}/{lastPage}</span>
        <button type="button" disabled={result.page >= lastPage} onClick={() => onPage(result.page + 1)} className="min-h-9 rounded-lg border border-slate-200 px-3 disabled:opacity-40">Sau</button>
      </div>}
    </div>
  </div>;
};
...
const ReportDashboard: React.FC<{ dashboard: DashboardDefinition; definitions: ReportDefinition[]; metricsByKey: Map<ReportMetricKey, ReportMetricDefinition> }> = ({ dashboard, definitions, metricsByKey }) => {
  ...
  return <section data-testid="report-dashboard" className="rounded-2xl border border-slate-200 bg-slate-50 p-4 shadow-sm sm:p-5">
    ...
    {loading ? <p className="text-sm text-slate-500">Đang cập nhật dashboard…</p> : <div className="grid gap-3 lg:grid-cols-2">{widgets.map(definition => {
      ...
      return <article key={definition.id} className="rounded-xl border border-slate-200 bg-white p-4">
        <h4 className="text-sm font-bold text-slate-900">{definition.name}</h4>
        <p className="mt-3 text-2xl font-black text-[#006b68]">{outcome ? formatMetric(value, metric) : '—'}</p>
...
const MetricCard: React.FC<{ metric?: ReportMetricDefinition; value: number; onDrill: () => void }> = ({ metric, value, onDrill }) => <button type="button" onClick={onDrill} className="rounded-2xl border border-slate-200 bg-white p-4 text-left shadow-sm transition hover:border-[#006b68] hover:shadow-md">
  <div className="mb-3 flex h-9 w-9 items-center justify-center rounded-xl bg-teal-50 text-[#006b68]"><BarChart3 className="h-4 w-4" /></div>
  ...
  <div className="mt-1 text-[10px] font-semibold text-[#006b68]">Bấm để xem hồ sơ</div>
</button>;
...
const ReportBreakdown: React.FC<{
...
}> = ({ result, metrics, metricsByKey, groupLabel, sort, onSort, onDrill }) => <div className="min-w-0 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
  ...
  <div className="hidden overflow-x-auto md:block"><table className="w-full min-w-[680px] text-left text-xs">
    <thead className="bg-slate-50 text-[10px] font-bold uppercase tracking-wide text-slate-500"><tr>
      <th className="px-4 py-3">{groupLabel}</th>
      {metrics.map(key => <th key={key} className="px-4 py-3 text-right"><button type="button" onClick={() => onSort(key)} className="inline-flex items-center gap-1 font-bold uppercase tracking-wide hover:text-[#006b68]" aria-label={`Sắp xếp theo ${metricsByKey.get(key)?.label || 'chỉ số'}`}>{metricsByKey.get(key)?.label}<span aria-hidden className="text-[9px] text-[#006b68]">{sort?.key === key ? (sort.direction === 'asc' ? '▲' : '▼') : '↕'}</span></button></th>)}
    </tr></thead>
    <tbody className="divide-y divide-slate-100">{result.groups.map(row => <tr key={row.key} className="hover:bg-slate-50">
      ...
      {metrics.map(key => <td key={key} className="px-4 py-3 text-right"><button type="button" onClick={() => onDrill(row.key)} className="font-bold text-[#006b68] underline-offset-2 hover:underline">{formatMetric(row.metricValues[key] || 0, metricsByKey.get(key))}</button></td>)}
    </tr>)}</tbody>
  </table></div>
