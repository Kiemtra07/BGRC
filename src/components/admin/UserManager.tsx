...
const UserCard: React.FC<UserCardProps> = ({ user, compact = false, onAuthenticatorChange, updatingAuthenticator = false, onEdit, onDelete, onResetPassword }) => (
  <article className={`rounded-xl border border-slate-200 bg-white ${compact ? 'p-3' : 'p-4'} shadow-sm`}>
    ...
    {onAuthenticatorChange && (
      <label className="mt-3 flex cursor-pointer items-center justify-between gap-3 rounded-lg border border-violet-200 bg-violet-50 px-3 py-2 text-[11px] font-bold text-violet-900">
        <span className="flex items-center gap-2"><ShieldCheck className="h-3.5 w-3.5" />Google Authenticator</span>
...
export const UserManager: React.FC<Props> = ({ users, orgUnits, onUserCreated, onUsersImported, onAuthenticatorChange, onUserUpdated, onUserDeleted, onUserPasswordReset }) => {
  ...
  return (
    <div className="space-y-5" data-testid="admin-user-directory" aria-busy={Boolean(updatingUserId)}>
      ...
      {authenticatorSetup && (
        <div role="alert" className="rounded-xl border-2 border-violet-400 bg-violet-50 p-4 shadow-lg">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              <p className="flex items-center gap-2 text-xs font-black text-violet-950"><ShieldCheck className="h-4 w-4" />Đã bật Google Authenticator cho {authenticatorSetup.fullName}</p>
              <p className="mt-1 text-[11px] font-semibold text-violet-900">Lưu secret này hoặc nhập URI vào Google Authenticator. Secret chỉ hiển thị một lần.</p>
              <dl className="mt-3 grid gap-2 sm:grid-cols-2">
                <div className="rounded-lg border border-violet-300 bg-white p-2.5"><dt className="text-[9px] font-bold uppercase tracking-wide text-slate-500">Secret</dt><dd className="mt-1 select-all break-all font-mono text-sm font-bold text-slate-900">{authenticatorSetup.secret}</dd></div>
                <div className="rounded-lg border border-violet-300 bg-white p-2.5"><dt className="text-[9px] font-bold uppercase tracking-wide text-slate-500">otpauth URI</dt><dd className="mt-1 select-all break-all font-mono text-[10px] font-semibold text-slate-900">{authenticatorSetup.otpauthUri}</dd></div>
              </dl>
            </div>
            <button type="button" aria-label="Tôi đã lưu secret Authenticator" onClick={() => setAuthenticatorSetup(null)} className="shrink-0 rounded-lg p-1 text-violet-900 hover:bg-violet-100"><X className="h-4 w-4" /></button>
          </div>
        </div>
      )}
