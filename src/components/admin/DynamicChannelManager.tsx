...
export const DynamicChannelManager: React.FC<Props> = ({ channels, onChannelCreated, onChannelUpdated, onChannelDeleted }) => {
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-2xl border border-rule bg-white p-4 shadow-panel sm:p-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex flex-col gap-2 sm:flex-row">
            ...
            <button type="button" onClick={() => setEditor('NEW')} className="inline-flex items-center justify-center gap-2 rounded-lg bg-brand-500 px-3 py-2 text-xs font-bold text-white hover:bg-brand-600"><Plus className="h-4 w-4" /> Tạo loại báo cáo</button>
          </div>
        </div>
        <div className="relative mt-4 max-w-lg"><Search className="absolute left-3 top-2.5 h-4 w-4 text-slate-400" /><input value={search} onChange={event => setSearch(event.target.value)} placeholder="Tìm theo mã, tên hoặc đơn vị..." className="w-full rounded-lg border border-rule py-2 pl-9 pr-3 text-xs focus:border-brand-500 focus:outline-none" /></div>
      </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-rule bg-white shadow-panel">
        <div className="hidden grid-cols-[minmax(260px,1.4fr)_minmax(180px,1fr)_110px_110px_150px] gap-3 border-b border-rule bg-slate-50 px-4 py-3 text-[11px] font-bold text-slate-500 md:grid">
        ...
        <div className="divide-y divide-slate-100">
          {filtered.map(channel => (
            <div key={channel.id} className="grid grid-cols-1 gap-3 px-4 py-4 md:grid-cols-[minmax(260px,1.4fr)_minmax(180px,1fr)_110px_110px_150px] md:items-center">
              <div className="min-w-0"><div className="flex items-center gap-2"><span className="rounded-lg bg-brand-50 p-2 text-brand-600"><FileSpreadsheet className="h-4 w-4" /></span><div className="min-w-0"><p className="truncate text-sm font-bold text-slate-900">{channel.name}</p><p className="mt-0.5 font-mono text-[11px] font-bold text-brand-600">{channel.code}</p></div></div></div>
              ...
              <div className="flex items-center justify-start gap-1 md:justify-end">
                <button type="button" onClick={() => setEditor(channel)} className="inline-flex items-center gap-1 rounded-lg px-2 py-2 text-xs font-bold text-brand-600 hover:bg-brand-50"><Pencil className="h-4 w-4" /> Sửa</button>
