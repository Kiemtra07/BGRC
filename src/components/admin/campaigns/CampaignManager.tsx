...
import { CalendarRange, Check, FolderCog, Plus, UsersRound } from 'lucide-react';
import { AuditCampaign, CreateAuditCampaignDTO, OrgUnit, ReportChannel, UserProfile } from '../../../../shared/contracts';
...
export const CampaignManager: React.FC<Props> = ({ campaigns, users, orgUnits, channels, onCreate, onProvisionDrive }) => {
  const [open, setOpen] = useState(false);
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [form, setForm] = useState({
    code: '', name: '', decisionNo: '', startDate: '', endDate: '', leadUserId: '',
    memberIds: [] as string[], branchCodes: [] as string[], reportChannelIds: [] as string[],
  });
  const leaders = users.filter(user => user.roles.some(role => ['SUPERVISOR', 'INTERNAL_APPROVER'].includes(role)));
  const memberOptions = users.filter(user => user.roles.includes('INTERNAL_OFFICER'));
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      await onCreate({
        code: form.code, name: form.name, decisionNo: form.decisionNo, startDate: form.startDate, endDate: form.endDate,
      ...
      setOpen(false);
    } catch (reason) { setError(reason instanceof Error ? reason.message : 'Không thể tạo chuyên đề.'); }
  ...
  return <section className="space-y-4">
    <div className="flex flex-wrap items-center justify-between gap-3">
      ...
      <button type="button" onClick={() => setOpen(value => !value)} className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-[#006b68] px-4 text-xs font-bold text-white"><Plus className="h-4 w-4" />Tạo chuyên đề</button>
    </div>
    ...
    {open && <form onSubmit={submit} className="grid gap-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm lg:grid-cols-2">
      ...
      <div className="flex justify-end gap-2 lg:col-span-2"><button type="button" onClick={() => setOpen(false)} className="min-h-11 rounded-xl border border-slate-200 px-4 text-xs font-bold">Hủy</button><button disabled={busy} className="min-h-11 rounded-xl bg-[#006b68] px-4 text-xs font-bold text-white">{busy ? 'Đang tạo...' : 'Lưu chuyên đề'}</button></div>
    </form>}
    ...
    <div className="grid gap-3 lg:grid-cols-2">
      {campaigns.map(campaign => <article key={campaign.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <button type="button" onClick={() => onProvisionDrive(campaign.id)} disabled={campaign.driveProvisionStatus === 'PROVISIONING'} className="mt-4 inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-xl border border-[#006b68] px-3 text-xs font-bold text-[#006b68] disabled:opacity-50"><FolderCog className="h-4 w-4" />{campaign.driveProvisionStatus === 'PROVISIONING' ? 'Đang tạo kho...' : campaign.driveProvisionStatus === 'READY' ? <><Check className="h-4 w-4" />Đồng bộ lại quyền Drive</> : campaign.driveProvisionStatus === 'FAILED' ? 'Thử tạo lại kho dữ liệu' : 'Tạo kho dữ liệu'}</button>
      </article>)}
    </div>
  </section>;
};
