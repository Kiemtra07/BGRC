...
export function createWorkflowConfig(channelId: string, workflowType: 'ONE_TIER' | 'TWO_TIER'): DynamicWorkflowConfig {
  ...
  return { id: `workflow-${channelId || 'draft'}`, channelId, workflowType, stages: workflowType === 'ONE_TIER' ? [branch, headOffice] : [branch, control, headOffice] };
}
      ...
      {value.stages.map((stage, index) => (
        <React.Fragment key={stage.stageId}>
          <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
            ...
            <p className="mt-2 text-[11px] text-slate-500">{stage.allowedRoles.includes('BRANCH_INPUT') ? 'Cán bộ chi nhánh' : stage.allowedRoles.includes('BRANCH_CONTROLLER') ? 'Kiểm soát chi nhánh' : 'Người phê duyệt HT'}</p>
          </div>
