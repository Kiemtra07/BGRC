  ...
  return <section className="space-y-5" data-testid="report-form-cms">
    ...
    <div className="grid gap-4 lg:grid-cols-[210px_minmax(0,1fr)]">
      <aside className="rounded-xl border border-slate-200 bg-white p-3"><h4 className="text-xs font-extrabold text-slate-900">Thư viện block</h4><div className="mt-3 grid grid-cols-2 gap-2 lg:grid-cols-1">
        {([['CAMPAIGN_CONTEXT', BriefcaseBusiness], ['SECTION', Heading2], ['SUBSECTION', Heading3], ['TEXT', Text], ['FIELD', Plus], ['FIELD_GROUP', LayoutGrid], ['DIVIDER', Minus]] as const).map(([type, Icon]) => <button key={type} type="button" disabled={(type === 'FIELD' || type === 'FIELD_GROUP') && value.fields.length === 0} onClick={() => addBlock(type)} className="inline-flex min-h-10 items-center gap-2 rounded-lg border border-slate-200 px-3 py-2 text-left text-xs font-bold text-slate-700 hover:border-[#006b68] hover:bg-teal-50 disabled:opacity-40"><Icon className="h-4 w-4 text-[#006b68]" />{blockLabels[type]}</button>)}
      </div></aside>
