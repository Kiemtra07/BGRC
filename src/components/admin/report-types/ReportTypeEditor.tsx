...
export const ReportTypeEditor: React.FC<Props> = ({ channel, onClose, onSave }) => {
  ...
  const tabs = useMemo(() => [
    ...
    { id: 'FORM' as const, label: `Mẫu form (${draft.schemaConfig.formTemplate?.blocks.length ?? 0} block)`, icon: FileText },
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-950/55 p-0 sm:items-center sm:p-4">
      <div className="flex max-h-[96dvh] w-full max-w-6xl flex-col overflow-hidden rounded-t-2xl bg-white shadow-2xl sm:max-h-[92dvh] sm:rounded-2xl">
        ...
        <nav className="flex gap-2 overflow-x-auto border-b border-slate-200 px-4 py-2 sm:px-6">
          {tabs.map(tab => { const Icon = tab.icon; return <button key={tab.id} type="button" onClick={() => setActiveTab(tab.id)} className={`inline-flex shrink-0 items-center gap-2 rounded-lg px-3 py-2 text-xs font-bold ${activeTab === tab.id ? 'bg-[#006b68] text-white' : 'text-slate-600 hover:bg-slate-100'}`}><Icon className="h-4 w-4" />{tab.label}</button>; })}
        </nav>
...
const GeneralEditor: React.FC<{ value: CreateReportChannelDTO; onChange: (value: CreateReportChannelDTO) => void; isNew: boolean }> = ({ value, onChange, isNew }) => (
  <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
    ...
    <label className="text-xs font-bold text-slate-700">Mã loại báo cáo<input value={value.code} onChange={event => { const code = event.target.value.toUpperCase().replace(/[^A-Z0-9_]/g, '_'); onChange(isNew ? { ...value, code, schemaConfig: { ...value.schemaConfig, tableName: code.toLowerCase() } } : { ...value, code }); }} className="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 font-mono text-xs" /></label>
    <label className="text-xs font-bold text-slate-700">Tên loại báo cáo<input value={value.name} onChange={event => onChange({ ...value, name: event.target.value })} className="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 text-xs" /></label>
    <label className="text-xs font-bold text-slate-700">Nhóm nghiệp vụ<select value={value.category} onChange={event => onChange({ ...value, category: event.target.value as CreateReportChannelDTO['category'] })} className="mt-1 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs"><option value="REGULAR_AUDIT">Kiểm tra thường xuyên</option><option value="THEMATIC_AUDIT">Kiểm tra chuyên đề</option><option value="COMPLIANCE_AML">Tuân thủ và AML</option><option value="OPERATIONAL_RISK">Rủi ro vận hành</option><option value="CREDIT_INSPECTION">Kiểm tra tín dụng</option><option value="BRANCH_REPORT">Báo cáo chi nhánh</option></select></label>
    <label className="text-xs font-bold text-slate-700">Đơn vị ban hành<input value={value.issuingDepartment} onChange={event => onChange({ ...value, issuingDepartment: event.target.value })} className="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 text-xs" /></label>
    <label className="text-xs font-bold text-slate-700 md:col-span-2">Mô tả<textarea rows={3} value={value.description} onChange={event => onChange({ ...value, description: event.target.value })} className="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 text-xs" /></label>
    <div className="md:col-span-2"><span className="text-xs font-bold text-slate-700">Cách nhập dữ liệu</span><div className="mt-2 flex flex-wrap gap-3">{([['WEB_FORM', 'Nhập form'], ['EXCEL_IMPORT', 'Nhập Excel'], ['API', 'API']] as const).map(([method, label]) => <label key={method} className="inline-flex items-center gap-2 rounded-lg border border-slate-200 px-3 py-2 text-xs font-semibold"><input type="checkbox" checked={value.inputMethods.includes(method)} onChange={event => onChange({ ...value, inputMethods: event.target.checked ? [...value.inputMethods, method] : value.inputMethods.filter(item => item !== method) })} />{label}</label>)}</div></div>
