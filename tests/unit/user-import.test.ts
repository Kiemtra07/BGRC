...
describe('parseUserImportRows', () => {
  it('ships a readable Excel template with the canonical import headers', async () => {
    ...
    expect(rows[0]).toEqual([
      ...
      'Phòng/PGD', 'Trạng thái', 'Ghi chú', 'Email Google Workspace',
    ]);
  });

  it('maps a valid batch row, masks password mode and detects duplicate email', () => {
    const rows = [
      ['Tên đăng nhập', 'Họ và tên', 'Email', 'Email Google Workspace', 'Mật khẩu tạm', 'Cổng', 'Vai trò chính', 'Vai trò CoPlus', 'Mã đơn vị/nhóm nội bộ', 'Trạng thái'],
      ['canbo01', 'Cán bộ Một', 'canbo01@example.com', 'canbo01@workspace.example.com', 'MatKhauTam@123', 'INTERNAL', 'INTERNAL_OFFICER', 'CB1_KTGSTT', 'TEAM_01', 'ACTIVE'],
      ['canbo02', 'Cán bộ Hai', 'CANBO01@example.com', '', '', 'INTERNAL', 'INTERNAL_OFFICER', 'CB1_KTGSTT', 'TEAM_01', 'ACTIVE'],
    ];
    ...
    expect(preview[0]).toMatchObject({ passwordMode: 'PROVIDED', errors: [], payload: { internalTeamId: 'team-1', teamRole: 'MEMBER', coplusRole: 'CB1_KTGSTT', googleWorkspaceEmail: 'canbo01@workspace.example.com' } });
