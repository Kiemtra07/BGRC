...
function sanitizeDriveFolderSegment(value: string): string {
...
export class GoogleDriveAdapter {
  ...
  public generateCampaignEvidenceFolderPath(params: { cif: string; customerName?: string; errorCode: string }): string {
  ...
  public async createResumableUploadSession(params: { fileName: string; mimeType: string; fileSize: number; sha256Checksum: string; folderPath: string; findingId: string; rootFolderId?: string }): Promise<DriveResumableUploadSession> {
    ...
    const parentId = await this.ensureGoogleFolderPath(params.folderPath, params.rootFolderId); const driveFileId = await this.generateDriveFileId();
  ...
  public async completeResumableUpload(params: { driveFileId: string; fileName: string; mimeType: string; fileSize: number; sha256Checksum: string; folderPath: string; findingId: string; rootFolderId?: string }): Promise<DriveUploadResult> {
    this.requireGoogleMode(); const fileName = this.validateUploadMetadata(params.fileName, params.mimeType, params.fileSize); this.requireChecksum(params.sha256Checksum);
